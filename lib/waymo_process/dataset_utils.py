...
from waymo_open_dataset.utils import camera_segmentation_utils, frame_utils
...
def get_2d_road_segm_with_image(frame):
    ...
    (
        ...
        panoptic_label, seg.panoptic_label_divisor
    )
...
def save_2d_road_segm_from_frame(
    frame,
    frame_number,
    filename,
    folder=None,
...
def filter_and_save_data(
    ...
    image, points_road = visualize_segm_pointcloud_on_image(
        frame,
        cp_points_all_filtered_road,
        points_all_input_filtered_road,
        0,
        visualize=visualize,
    )

    _, points_all = visualize_segm_pointcloud_on_image(
        frame, cp_points_all_filtered, points_all_input_filtered, 0, visualize=visualize
    )
...
def get_3d_data_from_frame(
    ...
    _points_all_ri2 = np.concatenate(points_ri2, axis=0)  # noqa: F841
    # point labels
    point_labels_all = np.concatenate(point_labels, axis=0)
    _point_labels_all_ri2 = np.concatenate(point_labels_ri2, axis=0)  # noqa: F841
    # camera projection corresponding to each point
    cp_points_all = np.concatenate(cp_points, axis=0)
    _cp_points_all_ri2 = np.concatenate(cp_points_ri2, axis=0)  # noqa: F841
