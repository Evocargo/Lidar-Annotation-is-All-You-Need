...
def train(
    ...
    for batch_i, (input, target, _paths, shapes) in enumerate(train_loader):
        ...
        if num_iter < num_warmup:
            # warm up
            lf = (
                lambda x: ((1 + math.cos(x * math.pi / config.TRAIN.END_EPOCH)) / 2)
                * (1 - config.TRAIN.LRF)
                + config.TRAIN.LRF
            )  # cosine
            xi = [0, num_warmup]
            for j, x in enumerate(optimizer.param_groups):
                x["lr"] = np.interp(
                    ...
                    xi,
                    [
                        config.TRAIN.WARMUP_BIASE_LR if j == 2 else 0.0,
                ...
                if "momentum" in x:
                    x["momentum"] = np.interp(
                        num_iter,
                        xi,
                        [config.TRAIN.WARMUP_MOMENTUM, config.TRAIN.MOMENTUM],
                    )
...
def validate(
    ...
    for batch_i, (img, target, _paths, shapes) in tqdm(
            ...
            if config.TEST.PLOTS and not config.inference_visualization:
                ...
                for image_ind in range(nb):  # for each image in batch
                    ...
                    if config.vis_without_letterboxing:
                        img_with_predict = img_with_predict[
                ...
                if config.save_video:
                    res_images.append(
                        cv2.cvtColor(
                            np.array(img_with_predict, dtype=np.uint8), cv2.COLOR_BGR2RGB
                        )
                    )
