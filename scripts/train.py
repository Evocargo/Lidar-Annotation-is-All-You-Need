...
from lib.utils.utils import create_logger, get_optimizer, save_checkpoint
...
def main():
    ...
    if cfg.DATASET.MASKS_ONLY:
        ...
        datasets = [dataset1]
        datasets_fractions = [1.0]
    elif cfg.DATASET.LIDAR_DATA_ONLY:
        print("We will use only reprojected lidar segmentation masks")
        ...
        print("We will use both 2d masks and reprojected 3d data")
        dataset1 = SegmDataset3D(
            cfg=cfg,
            is_train=True,
            inputsize=cfg.MODEL.IMAGE_SIZE,
    ...
    for dataset_index, dataset_ in enumerate(datasets):
        ...
        print(f"Dataset {dataset_.name} with fraction {datasets_fraction} is created")
